...
def analyze_period(pages_data, queries_data):
...
def main():
    ...
    periods = {
        '04-29-2026': ('04-29-2026/performance_page.json', '04-29-2026/performance_query.json'),
        '05-09-2026': ('05-09-2026/performance_page.json', '05-09-2026/performance_query.json'),
        '05-23-2026': ('05-23-2026/performance_page.json', '05-23-2026/performance_query.json'),
    }

    results = {}
    for label, (p_file, q_file) in periods.items():
        ...
        results[label] = {
            'stats': analyze_period(pages_data, queries_data),
            'queries': {q['keys'][0]: q for q in (queries_data or [])},
            'pages': {p['keys'][0]: p for p in (pages_data or [])}
        }

    print("\n=== GLOBAL STATISTICS COMPARISON ===")
    print(f"{'Metric':<25} | {'04-29-2026 (30d)':<18} | {'05-09-2026 (30d)':<18} | {'05-23-2026 (14d)':<18}")
    print("-" * 85)
    metrics = [
        ('clicks', 'Total Clicks'),
    ...
    for key, name in metrics:
        ...
        print(f"{name:<25} | {v1:<18} | {v2:<18} | {v3:<18}")
    ...
    print(f"{'Query':<55} | {'04-29-2026':<12} | {'05-09-2026':<12} | {'05-23-2026':<12}")
    print("-" * 100)
    for q_name in target_queries:
        cells = []
        for period in ['04-29-2026', '05-09-2026', '05-23-2026']:
            q_info = results[period]['queries'].get(q_name)
            if q_info:
                cells.append(f"{q_info['impressions']} imp / {q_info['position']:.1f}")
            else:
                cells.append("N/A")
        print(f"{q_name:<55} | {cells[0]:<12} | {cells[1]:<12} | {cells[2]:<12}")

    # Check learn pages vs services pages
    print("\n=== PAGE GROUP ANALYSIS ===")
    for label in ['04-29-2026', '05-09-2026', '05-23-2026']:
