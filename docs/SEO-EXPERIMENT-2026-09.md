...
The initial treatment URLs are:
...
Important Cedar Park query baselines from the reviewed export:
...
The previous template emitted `Service` structured data with Foundation Risk Registry as the `provider` of `Foundation Repair`. The current verified product behavior is an information/evaluation/lead flow, not enough evidence to assert that the Registry itself performs repair work.

The treatment implementation therefore uses `WebPage`, `Organization`, `FAQPage`, and `BreadcrumbList` entities. Do not restore provider/service assertions without documenting the real-world service relationship.
...
Any change to treatment membership, title/H1 wording, CTA, or measurement window should be documented here in the same commit or PR.